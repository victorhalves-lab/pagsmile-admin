...
function V8KPI({ eyebrow, value, sub, accent }) {
  return (
    <div className="v8-card" style={{ padding: '20px 22px', display: 'flex', flexDirection: 'column', gap: 8 }}>
      ...
      <span className="v8-num" style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-0.028em', color: accent || 'var(--v8-fg-strong)', lineHeight: 1.04 }}>
        {value}
      </span>
...
function PendingActionRow({ action }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid var(--v8-bd-subtle)' }}>
      <div style={{ width: 32, height: 32, borderRadius: 12, background: 'var(--v8-bg-warning)', color: 'var(--v8-fg-warning)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
        <Clock size={14} strokeWidth={2} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4, flexWrap: 'wrap' }}>
          <span className="v8-pill v8-pill--warning"><span className="v8-pill__dot" />Pendente</span>
          <span style={{ fontSize: 13, fontWeight: 600, color: 'var(--v8-fg-strong)', letterSpacing: '-0.014em' }}>{action.action_label}</span>
        </div>
        <div style={{ fontSize: 11, color: 'var(--v8-fg-muted)' }}>{action.target_summary} · iniciado por {action.actor.name}</div>
      </div>
      <div style={{ textAlign: 'right' }}>
        <div className="v8-num" style={{ fontSize: 18, fontWeight: 700, color: secondsLeft <= 10 ? 'var(--v8-fg-danger)' : 'var(--v8-fg-warning)' }}>
          0:{String(secondsLeft).padStart(2, '0')}
        </div>
        <div style={{ fontSize: 10, color: 'var(--v8-fg-subtle)', fontFamily: 'JetBrains Mono, monospace' }}>até confirmar</div>
      </div>
    </div>
  );
}
...
function LegacyEventRow({ event }) {
  ...
  return (
    <div style={{ display: 'flex', gap: 12, padding: '14px 16px', borderBottom: '1px solid var(--v8-bd-subtle)' }}>
      <div style={{ width: 36, height: 36, borderRadius: 12, background: 'var(--v8-bg-surface-2)', color: 'var(--v8-fg-muted)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
        <User size={16} strokeWidth={1.8} />
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 6 }}>
          <span className={`v8-pill ${sev}`}>{event.severity}</span>
          <code style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, fontWeight: 600, color: 'var(--v8-fg-strong)', background: 'var(--v8-bg-surface-2)', padding: '2px 7px', borderRadius: 6, border: '1px solid var(--v8-bd-subtle)' }}>
            {event.action}
          </code>
          <span style={{ color: 'var(--v8-fg-subtle)', fontSize: 11 }}>→</span>
          <code style={{ fontFamily: 'JetBrains Mono, monospace', fontSize: 11, color: 'var(--v8-fg-muted)' }}>{event.target}</code>
        </div>
        <div style={{ fontSize: 11, color: 'var(--v8-fg-muted)' }}>
          <strong style={{ color: 'var(--v8-fg-strong)' }}>{event.user}</strong>{' · '}
          <span style={{ fontFamily: 'JetBrains Mono, monospace' }}>{new Date(event.timestamp).toLocaleString('pt-BR')}</span>
        </div>
        {event.diff && (
...
function V8Select({ value, onChange, options }) {
  return (
    <select
      ...
      style={{
        height: 36, padding: '0 32px 0 12px', background: 'var(--v8-bg-surface)',
    ...
    <div data-ds="v8" style={{ minHeight: '100vh', background: 'var(--v8-bg-canvas)' }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          ...
          actions={
            <button
              ...
              style={{
                ...
                color: 'var(--v8-fg-strong)', cursor: 'pointer', transition: 'all .14s',
              }}
              onMouseEnter={e => { e.currentTarget.style.background = 'var(--v8-bg-brand-soft)'; e.currentTarget.style.borderColor = 'var(--v8-bd-brand)'; e.currentTarget.style.color = 'var(--ps-700)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = 'var(--v8-bg-surface)'; e.currentTarget.style.borderColor = 'var(--v8-bd-default)'; e.currentTarget.style.color = 'var(--v8-fg-strong)'; }}
            >
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16 }}>
          <V8KPI eyebrow="EVENTOS HOJE" value={mockUndoStats.total_today + LEGACY_AUDIT.length} sub={`${mockUndoableActionsHistory.length} undo-able · ${LEGACY_AUDIT.length} clássicos`} />
          <V8KPI eyebrow="CONFIRMADAS" value={`${mockUndoStats.confirmed_pct}%`} sub="das ações undo-able" accent="var(--v8-fg-success)" />
          <V8KPI eyebrow="DESFEITAS" value={`${mockUndoStats.cancelled_pct}%`} sub="canceladas no undo window" accent="var(--v8-fg-info)" />
          <V8KPI eyebrow="PENDENTES AGORA" value={mockUndoStats.pending_now} sub="aguardando confirm/desfazer" accent={mockUndoStats.pending_now > 0 ? 'var(--v8-fg-warning)' : 'var(--v8-fg-muted)'} />
        </div>

        {mockPendingActionsQueue.length > 0 && (
          <div className="v8-card" style={{ padding: 0, overflow: 'hidden' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '14px 16px', borderBottom: '1px solid var(--v8-bd-default)', background: 'var(--v8-bg-surface-2)' }}>
            ...
            {mockPendingActionsQueue.map(a => <PendingActionRow key={a.action_id} action={a} />)}
          </div>
        )}

        <div className="v8-card" style={{ padding: 16, display: 'flex', gap: 10, flexWrap: 'wrap', alignItems: 'center' }}>
          <div style={{ position: 'relative', flex: '1 1 280px', minWidth: 220 }}>
            <Search size={14} strokeWidth={2} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(-50%)', color: 'var(--v8-fg-subtle)', pointerEvents: 'none' }} />
            <input
              type="text"
              value={search}
              ...
              onFocus={e => { e.currentTarget.style.borderColor = 'var(--ps-500)'; e.currentTarget.style.boxShadow = 'var(--sh-focus)'; }}
              onBlur={e => { e.currentTarget.style.borderColor = 'var(--v8-bd-default)'; e.currentTarget.style.boxShadow = 'none'; }}
            />
          </div>
        ...
        <div className="v8-card" style={{ padding: 0, overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', borderBottom: '1px solid var(--v8-bd-default)', background: 'var(--v8-bg-surface-2)' }}>
            <span className="v8-eyebrow">TIMELINE · {filteredHistory.length + filteredLegacy.length} eventos</span>
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', fontSize: 11, color: 'var(--v8-fg-muted)' }}>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><CheckCircle2 size={12} style={{ color: 'var(--v8-fg-success)' }} />confirmado</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><XCircle size={12} style={{ color: 'var(--v8-fg-muted)' }} />desfeito</span>
              <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><AlertTriangle size={12} style={{ color: 'var(--v8-fg-danger)' }} />erro</span>
            </div>
          </div>
          ...
          {filteredHistory.length === 0 && filteredLegacy.length === 0 ? (
            <div style={{ padding: 60, textAlign: 'center', color: 'var(--v8-fg-muted)' }}>
              ...
              <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--v8-fg-strong)', marginBottom: 4 }}>Nenhum evento encontrado</div>
              <div style={{ fontSize: 12 }}>Ajuste filtros ou busca para ver resultados.</div>
            </div>
          ) : (
            <>
              {filteredHistory.map(action => <UndoActionTimelineItem key={action.event_id} action={action} />)}
              {filteredLegacy.length > 0 && (
                <>
                  <div style={{ padding: '8px 16px', background: 'var(--v8-bg-surface-2)', borderTop: '1px solid var(--v8-bd-default)', borderBottom: '1px solid var(--v8-bd-default)' }}>
                    <span className="v8-eyebrow" style={{ color: 'var(--v8-fg-subtle)' }}>EVENTOS CLÁSSICOS (legado)</span>
                  </div>
                  {filteredLegacy.map(event => <LegacyEventRow key={event.id} event={event} />)}
                </>
              )}
            </>
          )}
      ...
      <style>{`
        [data-ds="v8"] .v8-audit-item:hover {
          background: var(--v8-bg-brand-tint);
        }
      `}</style>
    </div>
  );
