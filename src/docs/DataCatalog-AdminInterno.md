# Catálogo de Dados — Visão Admin Interno (PagSmile)

> Governança, operações e visão financeira completa da PagSmile.
> Inclui dados confidenciais: custo de adquirente, margem, conciliação 3-way e governança.

---

## 1. Dashboard Executivo Interno

### Dinheiro Agora
- **Saldo Disponível** — Valor líquido pronto para operação, já liquidado e sem bloqueios. Expresso em R$.
- **Pendente (A Liberar)** — Valor já aprovado nas vendas dos merchants, mas ainda em período de retenção antes de transitar para o saldo disponível. Expresso em R$.
- **Bloqueado** — Valor retido por disputas em aberto, chargebacks, bloqueios regulatórios ou ordens judiciais. Expresso em R$.

### Antecipação
- **Recebível Disponível para Antecipar** — Soma dos recebíveis futuros dos merchants que atendem aos critérios de elegibilidade para antecipação. Expresso em R$.
...
- **D+0 / D+1 / D+7 / D+15 / D+30** — Distribuição dos recebíveis por janela de liquidação (quantos dias faltam para cair). Cada bucket mostra o valor em R$ e a quantidade de recebíveis.
- **% por Bucket** — Participação de cada janela no total a receber. Expresso em %.

### GMV (Gross Merchandise Volume) — Consolidado PagSmile
- **GMV Hoje** — Soma do valor de todas as transações aprovadas no dia atual, considerando todos os merchants. Expresso em R$.
...
- **GMV por Merchant** — Distribuição do GMV consolidado por merchant ou grupo de merchants. Expresso em R$ e %.

### Indicadores de Performance — Plataforma
- **Taxa de Aprovação** — Proporção de transações aprovadas sobre o total processado na plataforma. Expresso em %.
...
### Risco Financeiro — Plataforma
- **Chargeback Ratio** — Proporção de chargebacks sobre as transações aprovadas em toda a plataforma. Expresso em %.
- **MED Ratio** — Proporção de MEDs sobre as transações PIX. Expresso em %.
- **Reserva Bloqueada** — Valor total retido em rolling reserve (reserva de garantia) de todos os merchants. Expresso em R$.

### Metas
- **Meta de GMV** — Objetivo de volume de vendas definido para a plataforma no período. Expresso em R$.
...
### Fluxo de Dinheiro
- **Waterfall Entradas/Saídas** — Visualização das entradas (vendas, liquidações) versus saídas (taxas, estornos, saques, repasses a adquirentes). Expresso em R$.
- **Breakdown por Canal** — Distribuição do volume por canal de captura (e-commerce, recorrente, POS). Expresso em R$ e %.
...
- **Custo por Adquirente** — Custo total pago a cada processador no período. Expresso em R$.
- **Margem por Adquirente** — Diferença entre receita cobrada do merchant e custo pago ao adquirente. Expresso em R$ e %.

### Alertas
- **Alertas Críticos** — Itens que exigem ação imediata (disputas críticas, divergências de conciliação, falhas de webhook, bloqueios regulatórios). Expresso em quantidade.
- **Alertas com Snooze** — Itens adiados temporariamente pelo usuário. Expresso em quantidade.
...
- **Análise de Cohort** — Retenção de clientes agrupados por mês de aquisição. Expresso em %.
- **Heatmap por Hora/Dia** — Concentração de transações por hora × dia da semana. Expresso em quantidade.

---

## 2. Transações — Lista (Visão Interna)

- **ID da Transação** — Identificador único da transação na plataforma. Exibido de forma truncada, acompanhado de badges indicadores (antifraude, split, recorrente, chargeback).
- **Vendedor** — Nome do merchant ou subconta responsável pela venda. Visível na visão interna PagSmile, com nome e e-mail.
...
- **Data de Criação** — Data e hora em que a transação foi criada na plataforma.
- **Data de Liberação** — Data prevista para o valor ser liquidado ao merchant. Segue o prazo padrão de liquidação do meio de pagamento.
- **Data de Retenção** — Data prevista para liberação do rolling reserve (reserva de garantia retida temporariamente).
- **TPV** — Valor total da transação (Total Payment Volume). Expresso em R$.
...
- **Liquidação do Adquirente** — Valor recebido do adquirente após desconto do custo do processador. Expresso em R$.
- **Receita PagSmile** — Receita cobrada do merchant pela plataforma (MDR + antifraude + 3DS + gateway). Expresso em R$.
- **Custo do Adquirente** — Custo pago ao processador de pagamentos. Expresso em R$.
- **Margem PagSmile** — Diferença entre receita cobrada e custo pago ao adquirente. Expresso em R$ e %.
- **Total de Taxas** — Soma de todas as taxas cobradas na transação. Expresso em R$ e % do TPV.
...
- **ID do Pedido** — Referência do pedido informada pelo merchant.
...
- **Volume Aprovado (TPV)** — Soma dos valores das transações aprovadas. Expresso em R$.
- **Receita Total PagSmile** — Soma das receitas cobradas de todos os merchants. Expresso em R$.
- **Custo Total de Adquirentes** — Soma dos custos pagos a todos os processadores. Expresso em R$.
- **Margem Total** — Diferença entre receita e custo. Expresso em R$ e %.
- **Chargebacks** — Quantidade de transações em chargeback.

---

## 3. Transações — Detalhe (Visão Interna)
...
- **Webhook Enviado** — Notificação enviada ao merchant, com URL, status HTTP e tempo de resposta.
...
**Valores e Taxas Decompostos (Visão Interna):**
- **TPV** — Valor total da transação. Expresso em R$.
- **MDR** — Taxa de desconto principal cobrada do merchant. Expresso em R$ e %.
...
- **Receita PagSmile** — Soma de todas as taxas cobradas do merchant. Expresso em R$.
- **Custo do Adquirente** — Custo pago ao processador pela transação. Expresso em R$.
- **Margem PagSmile** — Receita menos custo. Expresso em R$ e %.
- **Total de Taxas** — Soma de todos os componentes. Expresso em R$ e % do TPV.
- **Data de Liquidação** — Data prevista para liberação ao merchant.
...
- ID PagSmile, ID do pedido do merchant, TID (identificador da transação no adquirente, cartão) e ARN (número de referência do adquirente, cartão).

**Metadata:**
- JSON completo enviado pelo merchant na criação da transação.
...
### Tab: Eventos & Logs
- **Feed de Eventos** — Lista cronológica de eventos internos, com filtros por tipo (Sistema, Antifraude, Adquirente, Webhooks):
  - **Transação Criada** — origem, merchant e chave de idempotência.
  ...
  - **Adquirente Autorizou** — adquirente, código de autorização, NSU e código de retorno.
...
**Eventos Técnicos Adicionais (Visão Interna):**
- Logs de infraestrutura, retries, timeouts e eventos de sistema não visíveis ao merchant.
...
- **Webhook Enviado** — JSON do evento enviado ao merchant (tipo do evento, dados, ID de entrega).
...
**Cards de Governança (Exclusivos Visão Interna):**
- **Conciliação PagSmile × Adquirente** — Comparativo de status, valor e data entre os dois lados, com indicador de match e histórico de conciliações.
- **Eventos Técnicos** — Logs detalhados de infraestrutura e retries.
- **Comprovante** — Cupom visual da transação, com opção de reimpressão e envio ao pagador.

---

## 4. Links de Pagamento (Governança)

### Lista
- **Nome do Link** — Identificador do produto ou link de pagamento.
- **Merchant** — Dono do link. Expresso em nome e e-mail.
...
### KPIs
- **Total de Links** — Quantidade de links criados na plataforma.
...
## 5. Recusas (Visão Operacional)

### Lista
- **ID da Transação** — Identificador da transação recusada.
- **Merchant** — Nome do merchant cuja transação foi recusada.
...
- **Recusas por Merchant** — Ranking de merchants com maior taxa de recusa. Expresso em % e quantidade.

---

## 6. Estornos (Visão Operacional)

### Lista
- **ID Original** — Transação de origem do estorno.
- **ID do Estorno** — Identificador do estorno.
- **Merchant** — Nome do merchant que solicitou o estorno.
...
- **Estornos por Merchant** — Ranking de merchants por volume de estornos. Expresso em R$ e quantidade.
- **Tempo Médio** — Tempo entre solicitação e processamento. Expresso em horas ou dias.

---

## 7. Disputas / Chargebacks / MEDs / Pré-Chargebacks (Governança)
...
- **Merchant** — Nome do merchant envolvido.
...
- **Disputas por Merchant** — Ranking de merchants com maior volume de disputas. Expresso em R$ e quantidade.

---

## 8. Recebíveis (Governança)

### Lista
- **Data de Liquidação** — Data prevista para liberação, com indicador de quantos dias faltam (D+X).
- **Merchant** — Nome do merchant dono do recebível.
- **Transação** — ID da transação vinculada.
...
- **Líquido** — Valor que será efetivamente repassado ao merchant. Expresso em R$.
...
- **Merchant** — Dono do recebível cedido.
...
- **Concentração** — Distribuição por meio, bandeira e merchant. Expresso em %.
...
## 9. Antecipação (Governança)

### Lista
- **Disponível para Antecipar** — Soma dos recebíveis futuros elegíveis de todos os merchants. Expresso em R$.
- **Nº de Recebíveis** — Quantidade de recebíveis elegíveis.
- **Taxa Atual** — Custo percentual mensal aplicado. Expresso em % ao mês.
- **Prazo de Crédito** — Tempo estimado para o valor cair na conta do merchant.

### Tab: Histórico
- **Data** — Quando a antecipação foi realizada.
- **Merchant** — Quem solicitou a antecipação.
- **Valor Bruto** — Valor antecipado. Expresso em R$.
- **Taxa** — Custo da antecipação. Expresso em R$.
- **Líquido** — Valor efetivamente recebido pelo merchant. Expresso em R$.
- **Status** — Concluída.
...
### Análise
- **Taxa Efetiva Média** — Média do custo total das transações em relação ao valor processado. Expresso em %.
- **Benchmarks de Mercado** — Comparação da taxa efetiva com a média do setor. Expresso em %.
- **Distribuição por Adquirente** — Participação de cada processador no volume. Expresso em %.
- **Receita por Tipo de Taxa** — Decomposição da receita PagSmile por componente (MDR, gateway, antifraude, 3DS). Expresso em R$ e %.
...
## 11. Extrato Financeiro (Visão Interna)

### Lista
- **Data** — Data e hora do lançamento.
- **Tipo** — Crédito (entrada) ou débito (saída).
- **Categoria** — Tipo do lançamento (venda, estorno, chargeback, saque, antecipação, taxa, ajuste, split, repasse a adquirente).
...
## 12. Saques (Governança)

### Lista
- **ID** — Identificador do saque.
- **Merchant** — Nome do merchant que solicitou o saque.
...
## 13. Liquidações (Governança)

### Lista
- **ID** — Identificador da liquidação.
- **Merchant** — Nome do cliente que recebeu a liquidação.
...
- **Divergentes** — Quantidade de liquidações com divergência de conciliação.
...
- **Health Score** — Score de 0 a 100 baseado na qualidade da conciliação e ausência de divergências.
- **Análise Comparativa** — Comparação com período anterior. Expresso em %.
- **Governança** — Aprovações e trilha de auditoria.
- **Conciliação 3-way** — Comparação entre PagSmile, adquirente e banco. Estados: conciliado nos três lados, não conciliado ou divergente.

> **Regra de líquido:** Líquido = bruto - taxas - estornos + ajustes.
> **Conciliação 3-way:** Compara o valor registrado na PagSmile com o arquivo do adquirente e com o movimento bancário (via Lina/Open Finance).

---

## Apêndice — Métricas Derivadas (Visão Interna)
...
- **Margem PagSmile** — (Receita - custo) ÷ receita × 100. Em %.
- **Receita por Merchant** — Soma das receitas cobradas de cada merchant. Em R$.
- **Custo por Adquirente** — Soma dos custos pagos a cada processador. Em R$.
...
> **Fim do Catálogo — Visão Admin Interno.** Esta visão inclui todos os dados confidenciais da PagSmile: custo de adquirente, margem, conciliação 3-way e governança operacional.