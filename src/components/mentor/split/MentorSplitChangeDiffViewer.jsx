    ...
    <Card>
      <CardHeader className="pb-3">
        ...
        <div className="flex gap-2 flex-wrap pt-1">
          {grouped.high.length > 0 && <Badge className={SENSITIVITY_LABELS.high.color}>{grouped.high.length} alta</Badge>}
          {grouped.medium.length > 0 && <Badge className={SENSITIVITY_LABELS.medium.color}>{grouped.medium.length} média</Badge>}
          {grouped.low.length > 0 && <Badge className={SENSITIVITY_LABELS.low.color}>{grouped.low.length} baixa</Badge>}
        </div>
      </CardHeader>
      <CardContent className="space-y-2">
        {changes.map((c, i) => (
          <div key={i} className={cn('rounded-lg p-3 border-l-4', SENSITIVITY_COLORS[c.level])}>
