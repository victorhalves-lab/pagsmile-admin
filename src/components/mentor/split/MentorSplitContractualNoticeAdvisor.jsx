...
/**
 * Detector inédito no mercado · Mentor F2915.
 * Avalia se mudanças exigem aviso prévio contratual e calcula data sugerida.
 */
export default function MentorSplitContractualNoticeAdvisor({
  ...
  // Data mínima sugerida = hoje + noticeDays
  const today = new Date();
  const minDate = new Date(today);
  minDate.setDate(today.getDate() + noticeDays);

  // Verifica se a data proposta respeita o aviso prévio
      ...
      <CardContent className="space-y-3 text-sm">
        ...
        {contractClause?.has_clause && (
          <div className="bg-white rounded-lg p-2.5 border">
            ...
            <p className="text-xs text-slate-700 dark:text-slate-300 mt-1">{contractClause.contract_reference}</p>
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            ...
            <div>
              ...
              <p className="text-[10px] opacity-60">
                {proposed
                  ? `${daysUntilProposed >= 0 ? 'em ' : 'há '}${Math.abs(daysUntilProposed)} dias`
          ...
          {!respectsNotice && proposed && (
            <p className="text-[11px] text-amber-800 mt-2 font-medium">
              Aplicar antes de {minDate.toLocaleDateString('pt-BR')} pode caracterizar quebra contratual e ensejar
              questionamentos. Recomendamos reagendar.
            </p>
          )}
        </div>
      </CardContent>
